...
#probeManifest <- getProbeManifest(db_host=name_and_db["TEST_HOST"], db_name=name_and_db["TEST_DB"], collection="httr_probe",fields='{"transcripts":0}')
probeManifest <- getProbeManifest(db_host=name_and_db["TEST_HOST"], db_name=name_and_db["TEST_DB"], collection="httr_probe")
...
for(sample_id in count_samples) {
  ...
  qc_doc <- countQC(counts_doc,proben=proben,bad_probes=bad_probes)
  st <- list()
  flag <- qc_doc$qc_flag
  names(flag) <- 'qc_flag'
  st <- append(st, flag) 
